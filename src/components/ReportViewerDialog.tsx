...
export function ReportViewerDialog({ open, onClose, title, period, initialPage = 0 }: Props) {
  ...
  const pages = [
    <section key="cover" className="flex h-full flex-col">
    ...
    <section key="body-1-3" className="h-full">
    ...
    <section key="body-4-5" className="h-full">
  ...
  return (
    <div className="fixed inset-0 z-[80] flex flex-col bg-slate-900/60">
      ...
      <div
        ...
        className="flex min-h-0 flex-1 items-center justify-center gap-3 px-3 py-6 sm:gap-4 sm:px-4"
      >
        <button
          type="button"
          disabled={!canPrev}
          ...
          className={cn(
            "flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-white/20 bg-slate-900/70 text-white shadow-lg transition",
            canPrev ? "hover:bg-slate-800" : "cursor-not-allowed opacity-30",
          )}
          aria-label="이전 페이지"
        >
        ...
        <div
          className="relative w-full max-w-[210mm] origin-center overflow-hidden rounded-lg border border-slate-200 bg-white shadow-2xl"
        ...
        >
          ...
          <div className="h-[min(72vh,250mm)] overflow-y-auto p-10 text-sm leading-relaxed text-slate-800">
            {pages[page]}
          </div>
        </div>
        ...
        <button
          ...
          className={cn(
            "flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-white/20 bg-slate-900/70 text-white shadow-lg transition",
            canNext ? "hover:bg-slate-800" : "cursor-not-allowed opacity-30",
          )}
          aria-label="다음 페이지"
        >
